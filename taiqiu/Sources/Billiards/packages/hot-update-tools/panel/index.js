        ...
        window.plugin = new window.Vue({
            ...
            methods: {
                ...
                _packageVersion() {
					...
					*/
                    let resPath = path.join(this.resourceRootDir, "assets");
                    this._packageDir(resPath, zip.folder("assets"));
                    // 打包的文件名
                ...
                _checkResourceRootDir(jsbDir) {
                    if (FileUtil.isFileExit(jsbDir)) {
						...
						let resPath = path.join(jsbDir, "assets");
                ...
                onCopyFileToLocalServer() {
					...
					this._copySourceDirToDesDir(resPath, path.join(this.localServerPath, "assets"));
                ...
                _getTotalCopyFileNum() {
					...
					let resNum = this._getFileNum(path.join(this.resourceRootDir, "assets"));
                ...
                onCleanSimRemoteRes() {
                    let path = require("fire-path");
                    let runPlatform = window.navigator.platform;//"Windows";//cc.sys.os;
                    let remoteAsset = null;
                    
                    // Editor.log(window.navigator.platform)
