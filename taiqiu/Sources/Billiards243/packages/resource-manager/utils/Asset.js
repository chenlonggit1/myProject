...
module.exports = 
{
    ...
    search(asset, uuid,searchType)
    {
        ...
        }else
        {
            ...
            //Editor.Utils.UuidUtils.decompressUuid("66cdb0TUvFBmbRJPkY60mNt");
            //Editor.Utils.UuidUtils.compressUuid("66cdb0TUvFBmbRJPkY60mNt");
            let jsonStr = Fs.readFileSync(jsonPath, {encoding:"utf8"});
            if(jsonStr.indexOf(uuid)==-1&&jsonStr.indexOf(Editor.Utils.UuidUtils.compressUuid(uuid))==-1)return null;
    ...
    findResources(arr,uuid,paths,type)
    {
        ...
        while((res=this.findUUID(arr,uuid,index))!=null)
        {
            index = res.index+1;
            let path = null;
            if(type=="prefab")path = this.findPrefabNodePath(res,arr,ids);
    ...
    findPrefabNodePath(res,arr,ids)
    {
        let json = res.json;
        let index = res.index;
        ...
        let childPath = "";
        let childID = -1;
        let vo = this.isHasPrefabRes(node,ids);
    ...
    isHasPrefabRes(json,arr)
    {
        if(arr.indexOf(json)!=-1)return null;
        arr.push(json);
        return {};
    },
    ...
    findUUID(arr,uuid,index)
    {
        for (let i = index; i < arr.length; i++) 
        {
            ...
            if(str.indexOf(uuid)==-1&&str.indexOf(Editor.Utils.UuidUtils.compressUuid(uuid))==-1)continue;
